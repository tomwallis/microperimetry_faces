...
\author[1,3]{Thomas S.~A.~Wallis
\thanks{Corresponding author: \\ 
Thomas Wallis \\
Present address: \\
Centre for Integrative Neuroscience and Department of Computer Science \\
The University of T\"{u}bingen \\
T\"{u}bingen, Germany \\
Email: thomas.wallis@uni-tuebingen.de} \thanks{These authors contributed equally}}
...
Facial features, such as the eyes \citep{sekuler2004inversion}, are the primary visual cues most humans use in discriminating people they know from those they do not.
...
Since photoreceptor and ganglion cell density \citep{curcio2004human,curcio1990topography} as well as contrast sensitivity \citep{robson1981probability,rovamo1978cortical,hilz1974functional,Kelly:1984uc,johnston1987spatial} decrease as retinal eccentricity increases, it is no surprise that patients with sensitivity losses in the central visual field have difficulty perceiving faces (see \citep{Kwon:2011cw} for a quantification of the human CSF contribution to face recognition in the fovea and periphery).
...
For example, in identity and expression recognition tasks in which viewing distance was manipulated, patients with central field loss performed worse (required a nearer viewing distance) than normally-sighted controls, and were generally better at recognising the expression of a face than its identity \citep{Bullimore:1991tp}.
In addition, word reading acuity was better correlated with face recognition thresholds than letter chart acuity, grating acuity and contrast threshold for edge detection.
...
Another study\citep{tejeria2002face} correlated measures of visual function (letter acuity, reading, Pelli-Robson contrast sensitivity and colour vision) and self-reported difficulty in face recognition with performance on a familiar face identification and an odd-one-out expression discrimination task in 30 subjects with AMD.
...
Certain types of digital image enhancement improved recognition performance in some (16 of 38) patients with central vision loss on a ``celebrity'' / ``not a celebrity'' face recognition task \citep{Peli:1991wv} (see also \citep{Peli:1994wd}).
Patients with central field loss also demonstrated poorer performance on a famous face recognition task relative to controls \citep{Dulin:2011gg}.
...
\begin{center}
\textbf{Figure \ref{fig:classification} goes about here.}
\end{center}

\begin{center}
\textbf{Table \ref{tab:demographic} goes about here.}
\end{center}
...
A detailed description of how the faces were photographed and processed is provided in\citep{Gaspar2008}, thus only a brief description will be provided here \footnote{The face stimuli used in the experiment are available upon request}.
...
\begin{center}
\textbf{Figure~\ref{fig:faces} goes about here.}
\end{center}
...
A zero-mean Gaussian pixel noise (R.M.S. contrast, $\sigma = 0.005$) was added to the test image on each trial to allow comparison with the ideal observer (see Supplementary Materials Section 1.1).
...
Unfortunately we were unable to save raw fixation coordinates due to computer malfunction, meaning that the more sensitive fixation measure of the Bivariate Contour Ellipse Area (BCEA; see \citep{Crossland:2009gf}) could not be calculated.
...
\begin{center}
\textbf{Table~\ref{tab:perimetry} goes about here.}
\end{center}
...
To analyse this data we used a multi-level logistic Generalised Linear Model (GLM; see Supplementary Materials Section 1.2 for details).
...
One way to consider a fixed effect is that it is a special case of a random effect, in which the variance parameter is set to be zero (i.e. there is no uncertainty around the point estimate of the parameter; see \citep{Gelman:2007te}, p. 245).
...
Convergence was assessed by the $\hat{R}$ value (the ratio of between-to-within chain variance; see \citep{Gelman:1992ts}) as well as by inspection of traceplots.
Code to perform these analyses and raw data can be found on the first author's Github page (
TODO INSERT URL).
...
In addition, to make the regression coefficients easier to interpret (see
\citep{Gelman:2007te} p.55), we centred each input predictor by subtracting the mean (or 2 in the case of block).
...
The full specification of the model is provided in the Supplementary Materials (Section 1.2), along with a discussion of the tolerance of our conclusions to the prior distributions chosen.
...
These are 95\% \textit{credible} intervals (as distinct from \textit{confidence} intervals \citep{Kruschke:2011uy}).
...
\begin{center}
\textbf{Figure~\ref{fig:subj_performance} goes about here.}
\end{center}
...
\begin{center}
\textbf{Figure~\ref{fig:group_performance} goes about here.}
\end{center}
...
The posterior distributions for the group-level parameters are provided in the Supplementary Materials (Figure~S2).
...
\begin{center}
\textbf{Figure~\ref{fig:mean_pc_differences} goes about here.}
\end{center}
...
However, these comparisons should be treated with caution since there were also no credible differences between the group slopes (see Supplementary Materials, Section~1.3), indicating that there is large uncertainty for these comparisons.
While all groups showed a credible learning effect (parameter $c$ was credibly greater than zero, see Supplementary Figure S2), there were no credible differences between group learning rates (Supplementary Figure S3).
...
That is, we \textit{a priori} assign higher probability to correlations around zero (see Supplementary Materials, Section 1.2.1), but with high uncertainty.

\begin{center}
\textbf{Table~\ref{tab:correlations} goes about here.}
\end{center}
...
\begin{center}
\textbf{Figure~\ref{fig:correlations} goes about here.}
\end{center}
...
\begin{center}
\textbf{Figure~\ref{fig:equiv_blur} goes about here.}
\end{center}
...
A scatterplot of fixation stability versus visual acuity (see Supplementary Materials, Figure~S4) shows two clusters, one for each of the patient groups, with little evidence of correlations within each group (though note that the LV:F group was selected on the basis of steady fixation).
...
for example, unstable fixation is associated with slower reading speeds \citep{Crossland:2004jq}.
Patients with retinitis pigmentosa (in which the macular is spared) and patients with Stargardt's dystrophy (in which the macular is diseased) performed equivalently poorly in a famous face recognition task despite showing markedly different visual acuity \citep{Dulin:2011gg}.
...
As many in the field of vision rehabilitation have stated (e.g. \citep{TrauzettelKlosinski:2010fr}), to offer appropriate vision rehabilitation practitioners must understand the impact of each patient's vision loss on various visual functions.
Our patient sample included patients with very different pathologies and visual function (see Table \ref{tab:demographic}).
While a variety of pathologies impair face recognition (see also \citep{Dulin:2011gg}), our results suggest the key determinant of face recognition performance was whether the fovea was preserved or impaired.
...
\caption{Stimuli used in the experiment (from \citep{Gaspar2008}, used with permission).
