...
\caption{Stimuli used in the experiment \citep[from][used with permission]{Gaspar2008}.
...
# load correlation samples from file to keep numbers the same as paper (uncomment above script to sample again):
library(plyr)
library(psybayes)
library(wutils)
library(xtable)
...
# load correlation samples (assumes /funs/compute_correlations.R has been run):
load(file=paste0(getwd(),'/output/correlations_samples.RData'))
